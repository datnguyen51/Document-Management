...
from app.controllers.api.statistic import (
    ...
    format_data_return_day_staff
)

URL_API = "/staff"
URL_API_LINE = "/staff-line"


@router.get(URL_API, dependencies=[Depends(JWTBearer)])
async def statistic_staff(request: Request, start: Optional[str] = None, end: Optional[str] = None, year: Optional[str] = None, db: Session = Depends(get_db)):
    try:
        jwt = JWTBearer()
        ...
        sum_document = db.query(StaffDocument).join(Document).filter(
        ...
        sum_document_finish = db.query(StaffDocument).join(Document).filter(
            StaffDocument.staff_id == record_staff.id).filter(
            Document.created_at >= start if start is not None else True).filter(
            Document.created_at <= end if end is not None else True).filter(
        ...
        sum_document_not_finish = db.query(StaffDocument).join(Document).filter(
            StaffDocument.staff_id == record_staff.id).filter(
            Document.created_at >= start if start is not None else True).filter(
            Document.created_at <= end if end is not None else True).filter(
            Document.status != DocumentStatus.FINISH).filter(
        ...
        sum_document_expire = db.query(StaffDocument).join(Document).filter(
            StaffDocument.staff_id == record_staff.id).filter(
            Document.created_at >= start if start is not None else True).filter(
            Document.created_at <= end if end is not None else True).filter(
        ...
        for data_record_document_type in records_document_type:
            ...
            data['value'] = db.query(StaffDocument).join(Document).filter(
                StaffDocument.document_type_id == data_record_document_type.id).filter(
                Document.created_at >= start if start is not None else True).filter(
                Document.created_at <= end if end is not None else True).filter(
                Document.deleted == False).count()
...
async def statistic_staff_line(request: Request, start: Optional[str] = None, end: Optional[str] = None, year: Optional[str] = None, db: Session = Depends(get_db)):
    try:
        jwt = JWTBearer()
        token = await jwt.__call__(request)
        user = decodeJWT(token)
        ...
        if start and end:
            ...
            list_data_return = format_data_return_day_staff(start, end, record_staff, db)
